...
function blockStyle(textarea: HTMLTextAreaElement, arg: StyleArgs): SelectionRange {
  ...
  if (hasReplaceNext && selectedText === replaceNext) {
    // reverting replaceNext selection and undoing formatting
    textarea.selectionStart = textarea.selectionEnd
  }
  ...
  if (
    ...
    if (arg.isHeader) {
      // headers are treated differently as there might be both removal and addition of prefix
...
function orderedList(textarea: HTMLTextAreaElement): SelectionRange {
  ...
  if (undoStyling) {
    ...
    // capturing current selection for further setup
